...
!> =============================================================================
    ...
    subroutine test_move(error)
        ...
        call act%move_to(100., 30.)
        call check(error, act%as_d(), "M100 30")
        call act%clear()
        call act%move_by(12.2, 30.)
        call check(error, act%as_d(), "m12.2 30")
    ...
    subroutine test_line(error)
        ...
        call act%line_to(100., 30.)
        call check(error, act%as_d(), "L100 30")
        call act%clear()
        call act%line_by(12.2, 30.)
        call check(error, act%as_d(), "l12.2 30")
        if (allocated(error)) return
